...
//
// LOCAL MOD: dropped `codec::Encode/Decode` and `scale_info::TypeInfo` derives.
// This crate is consumed off-chain only (CLI + WASM), not from a Substrate
// runtime, so SCALE codec traits are not needed. See vendored/blsag/PINNED_COMMIT.md.
...
//
// LOCAL MOD: dropped Substrate codec/scale-info derives (see PINNED_COMMIT.md).
